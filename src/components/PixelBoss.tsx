...
/**
 * PixelBoss 組件：渲染一個像素風格的關主
 * 每次生成隨一種預選的種子，讓每一關感覺都有專屬對手
 */
...
const PixelBoss: React.FC<PixelBossProps> = ({ index, className }) => {
    // 循環使用 SEEDS
    const seed = SEEDS[index % SEEDS.length];
    const url = `https://api.dicebear.com/8.x/pixel-art/svg?seed=${seed}&backgroundColor=b6e3f4,c0aede,d1d4f9`;
    ...
    return (
        <div className={`flex flex-col items-center ${className}`}>
            <img
                src={url}
                alt="Pixel Boss"
                className="nes-avatar is-large is-rounded boss-avatar"
                style={{ imageRendering: 'pixelated' }}
            />
            <p className="nes-badge">
                <span className="is-dark">BOSS: {seed}</span>
            </p>
        </div>
    );
};

export default PixelBoss;
